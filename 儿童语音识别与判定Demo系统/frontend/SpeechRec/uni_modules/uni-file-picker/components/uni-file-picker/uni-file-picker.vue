	...
	<view class="uni-file-picker">
		...
		<upload-file v-if="(fileMediatype !== 'image' || showType !== 'grid')" :readonly="readonly" :list-styles="listStyles"
		 :files-list="filesList" :showType="showType" :delIcon="delIcon" @uploadFiles="uploadFiles" @choose="choose" @delFile="delFile">
			<slot v-if="filesList.length == 0">
				<button type="primary" size="mini" style="overflow: auto;">选择</button>			
			</slot>
		</upload-file>
		<view v-if="filesList.length == 0 && title" class="uni-file-picker__header">
			{{title}}
			<!--
		...
		methods: {
			...
			async setSuccessAndError(res, fn) {
				...
				for (let i = 0; i < res.length; i++) {
					...
					if (index === -1 || !this.files || this.files.length == 0) break
